# oss-cursorRules

[English](#english) | [日本語](#japanese)

---

## English

A collection of projects that have Cursor Rules (`.cursor/rules`) prepared on GitHub.

### Project List

| Project Name | Repository                                                          | Description                                                  | Language/Framework          |
| ------------ | ------------------------------------------------------------------- | ------------------------------------------------------------ | --------------------------- |
| Supabase     | [supabase/supabase](https://github.com/supabase/supabase)           | PostgreSQL development platform for web, mobile, and AI apps | TypeScript, MDX, JavaScript |
| vecto3d      | [lakshaybhushan/vecto3d](https://github.com/lakshaybhushan/vecto3d) | A super simple tool to convert your SVG's to 3D models       | TypeScript                  |

### How to Add

When adding a new project, please include the following information:

- **Project Name**: Official project name
- **Repository**: GitHub repository URL
- **Description**: Project purpose and features (recommended within 50 characters)
- **Language/Framework**: Main technologies used

### Contributing

If you would like to add a new project to this list, please submit a pull request.

---

## 日本語

GitHub に Cursor Rules (`.cursor/rules`) が用意されているプロジェクトのまとめです。

### プロジェクト一覧

| プロジェクト名 | リポジトリ                                                          | 説明                                                            | 言語/フレームワーク         |
| -------------- | ------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------- |
| Supabase       | [supabase/supabase](https://github.com/supabase/supabase)           | PostgreSQL 開発プラットフォーム。Web、モバイル、AI アプリ構築用 | TypeScript, MDX, JavaScript |
| vecto3d        | [lakshaybhushan/vecto3d](https://github.com/lakshaybhushan/vecto3d) | SVG を 3D モデルに変換するシンプルなツール                      | TypeScript                  |

### 追加方法

新しいプロジェクトを追加する場合は、以下の情報を含めてください：

- **プロジェクト名**: プロジェクトの正式名称
- **リポジトリ**: GitHub リポジトリの URL
- **説明**: プロジェクトの目的や特徴（50 文字以内推奨）
- **言語/フレームワーク**: 主要な使用技術

### 貢献

このリストに新しいプロジェクトを追加したい場合は、プルリクエストを送信してください。
